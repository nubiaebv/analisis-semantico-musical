...
def _load_all():
    ...
    # TF-IDF
    tfidf_path = RESULTS / "tfidf_matrix.npz"
    genres_path = RESULTS / "generos.npy"
    if tfidf_path.exists() and genres_path.exists():
        _data_cache["tfidf"] = sp.load_npz(str(tfidf_path))
        _data_cache["genres_tfidf"] = np.load(str(genres_path), allow_pickle=True)

    # Word2Vec doc-embeddings
    w2v_path = RESULTS / "w2v_doc_embeddings.npy"
    if w2v_path.exists():
        _data_cache["w2v"] = np.load(str(w2v_path))

    # BERT embeddings
    bert_path = RESULTS / "bert_corpus_embeddings.npy"
    if bert_path.exists():
        _data_cache["bert"] = np.load(str(bert_path))
...
def _get_subset(key: str, n_max: int = 800):
    ...
    genres_key = "genres_tfidf" if key == "tfidf" else "genres_tfidf"
...
def load_comparison(_):
    ...
    for key in model_keys:
        ...
        try:
            ...
            if sp.issparse(X):
                pipe = Pipeline([("clf", LogisticRegression(max_iter=500, n_jobs=-1))])
            else:
                pipe = Pipeline([("sc", StandardScaler()),
                                 ("clf", LogisticRegression(max_iter=500, n_jobs=-1))])

            skf    = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
            scores = cross_val_score(pipe, X_d, y_enc, cv=skf, scoring="accuracy")
            clf_results[key] = {"mean": scores.mean(), "std": scores.std()}

            km  = KMeans(n_clusters=len(set(g)), random_state=42, n_init=10)
            lbl = km.fit_predict(X_d)
            n_s = min(len(X_d), 1000)
            idx = np.random.default_rng(0).choice(len(X_d), n_s, replace=False)
            sil = silhouette_score(X_d[idx], lbl[idx], metric="cosine")
            sil_results[key] = sil
        except Exception:
            pass

    # Stats
    avail = sum(1 for k in model_keys if k in clf_results)
    best_clf = max(clf_results, key=lambda k: clf_results[k]["mean"]) if clf_results else "—"
    best_sil = max(sil_results, key=lambda k: sil_results[k]) if sil_results else "—"

    stats = html.Div([
        stat_card(f"{avail}/3", "Modelos", "con datos disponibles", ACCENT1, "🤖"),
        stat_card(
            f"{clf_results[best_clf]['mean']:.1%}" if best_clf != "—" else "—",
            "Mejor accuracy", f"modelo: {best_clf.upper()}", ACCENT2, "🏆"),
        stat_card(
            f"{sil_results[best_sil]:.3f}" if best_sil != "—" else "—",
            "Mejor silhouette", f"modelo: {best_sil.upper()}", ACCENT3, "📊"),
        stat_card(f"{len(generos)}", "Géneros", "clases del corpus", "#10B981", "🎵"),
    ], className="stat-grid")

    # Accuracy chart
    def _bar_chart(values_dict, ylabel, color_map, yrange=None):
        labels = [m for k, m in zip(model_keys, modelos) if k in values_dict]
        vals   = [values_dict[k]["mean"] if isinstance(values_dict[k], dict) else values_dict[k]
                  for k in model_keys if k in values_dict]
        errs   = [values_dict[k]["std"] if isinstance(values_dict[k], dict) else None
                  for k in model_keys if k in values_dict]
        ...
        fig = go.Figure(go.Bar(
            ...
            text=[f"{v:.1%}" if isinstance(v, float) and v < 2 else f"{v:.3f}" for v in vals],
            textposition="outside",
            textfont=dict(size=11, family=FONT_MONO),
        ))
        fig.update_layout(**PLOTLY_LAYOUT, height=300,
                          yaxis=dict(title=ylabel, range=yrange, gridcolor=BORDER),
                          xaxis=dict(gridcolor="transparent"))
        return fig

    fig_acc = _bar_chart(clf_results, "Accuracy (5-fold CV)", {})
    fig_sil = _bar_chart(
        {k: v for k, v in sil_results.items()}, "Silhouette Score", {}, yrange=[0, 1]
    )

    # Tabla de métricas
    rows_html = []
    for key, label, color in zip(model_keys, modelos, model_colors):
        acc_val = f"{clf_results[key]['mean']:.4f} ± {clf_results[key]['std']:.4f}" if key in clf_results else "N/A"
        ...
        rows_html.append(html.Tr([
            html.Td(label, style={"color": color, "fontWeight": "700", "padding": "10px 14px",
            ...
            html.Td("sparse" if key == "tfidf" else ("150" if key == "w2v" else "768"),
...
def update_confusion(model_key):
    ...
    try:
        ...
        fig.update_layout(**PLOTLY_LAYOUT, height=380,
                          xaxis=dict(title="Predicho", gridcolor="transparent",
                                     tickfont=dict(size=9)),
                          yaxis=dict(title="Real", gridcolor="transparent",
                                     tickfont=dict(size=9)))
        return fig
...
def update_tsne_comparison(generos_sel):
    ...
    fig.update_layout(**PLOTLY_LAYOUT, height=380,
                      legend=dict(orientation="v", x=1.01, font=dict(size=9)))
    return fig
